...
function loadOptions(optionsLocation = defaultOptionsLocation) {
  const xmlHttpRequestStatus_DONE = 200;

  const xmlHttp = new XMLHttpRequest();
  xmlHttp.overrideMimeType("application/json");
  xmlHttp.open("GET", optionsLocation, true);
  xmlHttp.onreadystatechange = function () {
    if (xmlHttp.readyState == XMLHttpRequest.DONE) {
      if (xmlHttp.status == xmlHttpRequestStatus_DONE) {
...
function optionsParser(optionsLocation) {
  ...
  for (let i = 0; i < options.stat.list.length; i++) {
    ...
    document.getElementById(`${statCamelize}SaveDiv`).appendChild(saveCheckbox);
    saveCheckbox.type = "checkbox";
    saveCheckbox.name = `${statCamelize}SaveProficiency`;

    //Save modifier
    let saveModifier = document.createElement("input");
    document.getElementById(`${statCamelize}SaveDiv`).appendChild(saveModifier);
  ...
  for (let i = 0; i < options.damageTypes.length; i++) {
    ...
    for (let k = 0; k < IRVVector.length; k++) {
      ...
      document.getElementById(`${IRVName}Td${k + 1}`).appendChild(IRVCheckbox);
...
function verifyOptionsValidity(optionsLocation) {
  ...
  if (!invalid) {
    console.log(`Options file is valid!`);
  } else {
    console.log(`Options file is invalid! Refer to alert(s) that popped up.`);
  }
